...
import { sprintCrawlerFullBodyPresets, staleCrawlerFullBodyPresets } from './ZombieSprites'
...
export class Crawler extends Entity {
  ...
  public actions: Actions = {
    stale: { state: 'in-use', durationMs: 0, completeMs: 2000 },
    walk: { state: 'not-in-use', durationMs: 0, completeMs: 750 },
    jumpAttack: { state: 'not-in-use', durationMs: 0, completeMs: 2000 },
    die: { state: 'not-in-use', durationMs: 0, completeMs: 1000 }
  }

  constructor(position: Point, size = { height: spriteHeight * 4, width: spriteWidth *  4 }) {
  ...
  decideBodySprite(): {
    ...
    switch (true) {
      case this.actions.die.state === 'in-use': {
        return { presets: staleCrawlerFullBodyPresets, action: null }
      }
      case this.actions.die.state === 'complete': {
        return { presets: staleCrawlerFullBodyPresets, action: null }
      }
      case this.actions.walk.state === 'in-use': {
        return { presets: sprintCrawlerFullBodyPresets, action: this.actions.walk }
      }
      default:
        return { presets: staleCrawlerFullBodyPresets, action: this.actions.stale }
    }
  }
